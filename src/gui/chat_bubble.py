...
class ChatBubble(QHBoxLayout):
    def __init__(self, sender, message, parent):
        super().__init__()
        self.parent_window = parent
        self.sender = sender
        self.message = message

        if self.sender == self.parent_window.user_name:
    ...
    def init_ui(self):
        ...
        bubble_label.setStyleSheet("color: black; background-color: {}; border-radius: 15px; padding: 10px;".format('#E0F7FA' if self.sender == self.parent_window.user_name else '#E1FFC7'))

        bubble_layout.addWidget(sender_label)
        bubble_layout.addWidget(bubble_label)
        bubble_layout.addStretch(1)

        sender_icon = ClickableLabel(self.sender, self.icon_path)
